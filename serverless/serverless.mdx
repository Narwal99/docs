...
| **3. Restart Method**            | Can be restarted as long as not released.                    | Cannot be restarted after stopping. If the deployment has stopped, [create a new deployment](../api-reference/api-serverless#2-create-deployment); if not stopped, adjust the number of running containers by [setting replica count](../api-reference/api-serverless). |
| **4. Entering Container Method** | (Assumed to support JupyterLab, based on context).           | Have no JupyterLab entry; to enter the container, obtain SSH command and password (refer to [Documentation](../api-reference/api-serverless#7-list-container-events)). |
