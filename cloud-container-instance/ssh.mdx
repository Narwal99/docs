...
<Note>
Windows OS Users: You can log in using the built-in PowerShell/CMD. For a better terminal experience, it is recommended to download and use the [Cmder](https://cmder.app/) tool, which is ready to use without installation after extraction. Alternatively, the [XShell](https://www.xshell.com/en/free-for-home-school/) tool is an even better option.
</Note>

<Note>
Mac OS Users: You can complete the SSH login using the system's built-in Terminal.
</Note>
...
<Note>
In the ```ssh -p 10309 root@connect.nmb1.seetacloud.com``` command, the meanings of the parameters are as follows:
