---
title: 'GLM-5'
description: 'Use GLM-5 through AI Gateway with an OpenAI-compatible chat completions API.'
---

`GLM-5` is available through the OpenAI-compatible chat completions API. It is suitable for chat, reasoning, coding assistance, document understanding, and Chinese-English application workflows.
...
curl --location --request POST "https://api.gpuhub.com/api/v1/chat/completions" \
  ...
  --data-raw '{
    ...
    "messages": [
      {
        "role": "user",
        "content": "Hello! Please explain how to evaluate an LLM application."
      }
    ],
    "stream": true
  }'
...
stream = client.chat.completions.create(
    model="GLM-5",
    messages=[
        {"role": "user", "content": "Hello! Please explain how to evaluate an LLM application."}
    ],
    stream=True,
)
